...
export default function PortAdaptersClient({ adapters: initial, manufacturer, housingMounts, isSuperuser }: Props) {
    ...
    function openEdit(a: PortAdapter) {
        ...
        setProductIdInput(a.productId ?? '')
        setProductUrlInput(a.productUrl ?? '')
        setError(null)
        setModal('edit')
    }
    ...
    async function handleAdd() {
        ...
        try {
            const productPhotos = await uploadPhotoSlots(photos, '/api/admin/port-adapters/photos', { manufacturerSlug: manufacturer.slug })
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                {adapters.map((adapter) => (
                    <div key={adapter.id} className="group/card relative">
                        <Link
                            href={`/products/${manufacturer.slug}/port-adapters/${adapter.slug}`}
