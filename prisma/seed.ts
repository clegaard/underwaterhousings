...
async function main() {
    ...
    // Create camera rigs for the admin user
    const rigSonyA7IV = await prisma.cameraRig.create({
        data: {
    ...
    const rigIphone = await prisma.cameraRig.create({
        data: {
    ...
    await prisma.user.update({
        ...
        data: { defaultRigId: rigSonyA7IV.id },
    })
