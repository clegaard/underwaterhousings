...
async function main() {
    ...
    await prisma.cameraRig.deleteMany()
    ...
    // Gallery photos
    // Create camera rigs for the distinct equipment combinations used in gallery photos
    const rigA7RV = await prisma.cameraRig.create({
        data: {
            name: 'Sony A7R V + SeaFrogs SF-A7RV',
            cameraId: sonyA7RV.id,
            housingId: housingA7RV.id,
        }
    })

    const rigA7RVMacro = await prisma.cameraRig.create({
        data: {
            name: 'Sony A7R V + SeaFrogs SF-A7RV + FE 90mm Macro',
            cameraId: sonyA7RV.id,
    ...
    const rigA7RVWide = await prisma.cameraRig.create({
        data: {
            name: 'Sony A7R V + SeaFrogs SF-A7RV + FE 24-105mm',
            cameraId: sonyA7RV.id,
    ...
    const rigIphoneDiveVolk = await prisma.cameraRig.create({
        data: {
            name: 'iPhone 14 Pro + DiveVolk SeaTouch 4 Pro',
            cameraId: iphone14Pro.id,
    ...
    await prisma.galleryPhoto.createMany({
        data: [
            ...
            // iPhone 14 Pro photos through DiveVolk SeaTouch 4 Pro
            {
                ...
                height: 2028,
                userId: adminUser.id,
                cameraRigId: rigIphoneDiveVolk.id,
                focalLength: 2.2,
                aperture: 2.2,
                shutterSpeed: '1/540',
            },
